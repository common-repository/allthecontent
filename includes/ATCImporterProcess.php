...
if (!class_exists('ATCImporterProcess')) { class ATCImporterProcess {
    ...
    public function log($message) {
        error_log($message, 3, ABSPATH . 'atc_importer_log.txt');
    }
...
//        delete_option('atc-importer-processing');
    ...
    function on_success($content, $file) {
        ...
        if (isset($exists)) {
            ATCDB::createErrorLog($file, "Delivery " . $content['_deliveryId'] . " has already been imported!", null);
            return;
        }
        ...
        if (is_null($existingPost)) {
            ...
            if ($existingVersion >= intval($content['version'])) {
                ATCDB::createErrorLog($file, "Content " . $existingPost->ID . " version  " . $content['version'] . " has already been imported!", null);
                return;
            }
    ...
    public function format_content($post_id, array $content, $captions) {
        ...
        if (startsWith($content['refid'], 'tag:reuters')
            ...
            // insert reuters logo
            $formated .= "<img src='https://www.thomsonreuters.com/content/dam/ewp-m/images/image-library/en/logos/tr_h_lg_rgb_ps.jpg.transform/q90/image.jpg' width=164 alt='REUTERS'>";
        }
    ...
    public function format_content_credits(array $content) {
        ...
        if (startsWith($credits, "(c) Copyright Thomson Reuters")) {
            ...
            $sr = "<a href=\"https://www.allthecontent.com\" target=\"_blank\">/AllTheContent</a>";
    ...
    public function find_existing_post(array $content) {
        ...
        $query = "SELECT p.* FROM " . $postTableName . " p "
               . "  INNER JOIN " . $metaTableName . " pm ON p.ID = pm.post_id "
               . "  WHERE pm.meta_key = 'atc_refid' AND pm.meta_value = '" . $content['refid'] . "' "
               . "    AND p.post_type = 'post' AND p.post_status <> 'trash' AND p.post_status <> 'auto-draft'";
    ...
    public function find_existing_post_for_delivery($delivery_id) {
        ...
        $query = "SELECT p.* FROM " . $postTableName . " p "
               . "  INNER JOIN " . $metaTableName . " pm ON p.ID = pm.post_id "
               . "  WHERE pm.meta_key = 'atc_delivery_id' AND pm.meta_value = '" . $delivery_id . "' "
               . "    AND p.post_type = 'post' AND p.post_status <> 'trash' AND p.post_status <> 'auto-draft'";
        $row = $wpdb->get_row($query, OBJECT);
        return $row;
    }
    ...
    public function find_existing_attachment(array $att) {
        ...
        $postTableName = $wpdb->prefix . "posts";
        $query = "SELECT p.* FROM " . $postTableName . " p "
               . "  WHERE p.post_name = '". sanitize_text_field($att['uid']) . "' "
               . "    AND p.post_type = 'attachment' AND p.post_status <> 'trash' AND p.post_status <> 'auto-draft'";
        $row = $wpdb->get_row($query, OBJECT);
        return $row;
    }
